defmodule Subtitle.WebVTT do
  ...
  def unmarshal_text!(text) do
    case unmarshal_text(text) do
      {:ok, data} -> data
  ...
  defp tokenize_text(["<" | rest], acc) do
    {tag, rest} = consume_graphemes(rest, ">", [])
    tokenize_text(rest, [{:tag, Enum.join(tag)} | acc])
  end
  ...
  defp tokenize_text(rest, acc) do
    ...
    tokenize_text(rest, [{:text, Enum.join(text)} | acc])
  end
  ...
  defp parse_tokens([{:text, text} | t], {tag, nil}, acc) when tag in [:italics, :bold, :underline] do
